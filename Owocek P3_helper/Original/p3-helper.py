...
base = int(asyncio.run(get_address_hex()), 16)
...
# load data
with open('p2_item_table.dat', encoding="utf-8") as f:
    item_names = f.readlines()
    item_names = [line.rstrip() for line in item_names]
    
with open('p3_ins_table.dat', encoding="utf-8") as f:
    pac_funcs_raw = f.readlines()
    pac_funcs_raw = [line.rstrip() for line in pac_funcs_raw]

cur_class = 0
pac_buffer = []

for x in pac_funcs_raw:
    pac_func = x.split(',')
    ...
    async with websockets.connect(
        ...
        while True:
            ...
            if pac_id <= (len(pac_funcs[pac_class]) - 1):
                print(tm+" [PAC] "+hex(pac_addr)+ ": " + hex(pac_base) + " " + hex(pac_class) + " " + hex(pac_id) + " " + pac_funcs[pac_class][pac_id])
            else:
                print(tm+" [PAC] "+hex(pac_addr)+ ": " + hex(pac_base) + " " + hex(pac_class) + " " + hex(pac_id) + " UNK_"+str(hex(pac_class))+"_"+str(hex(pac_id)))
            #print(pac_funcs)
